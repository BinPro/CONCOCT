...
-c		Cluster file generated by CONCOCT. A comma-separated file with contig id:s in the first and cluster (bin) id:s in the second column.

-a		Annotation file. A tab-separated file with contig id:s in the first and feature id:s (e.g. COG id:s) in the second column. A contig may appear on multiple lines.

-m		Marker gene file. A file with marker feature id:s (e.g. COG:ids) in the first column. In the CONCOCT publication we used the file scg_cogs_min0.97_max1.03_unique_genera.txt
	
=head1 OPIONAL ARGUMENTS
	
-s		If the contig id:s in the first column of the annotation file have been concatenated with e.g. a number to indicate gene id, you can specify a separator character or string, in which case the string before the separator will be used as contig id. The character/string should be enclosed by "". To separate by space do -s "\s", by tab do -s "\t" and by punctuation do -s "\."  

-h 		Prints this help message.

-cov	Coverage file. A tab-separated file with contig id:s in first column and mean coverage of contigs in the different samples in subsequent columns. If this is specified, columns with mean coverages of the clusters in the different samples will be added after the SCG counts.     
